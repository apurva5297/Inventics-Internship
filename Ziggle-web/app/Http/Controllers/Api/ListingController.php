...
class ListingController extends Controller
{
    ...
    public function filter_store(Request $request,$slug)
    {
      ...
      {  
        ...
        foreach($all_products as $product)
        {
         ...
         if($request->min_price == null)
          ...
          $brand = json_decode($request->brand , true);
            ...
            if(count($brand) < 1)
            {
                $ids = Product::whereIn('id',$product_id)->pluck('id');
            }
            else
            {
                $ids = Product::whereIn('id',$product_id)->whereIn('manufacturer_id', $brand)->pluck('id');
            }

        if($attr == null)
        {
            $listings = Product::whereIn('id',$ids)->with(['inventories.image:path,imageable_id'])->withCount(['inventories' => function($query){
                $query->available();
            }])
            ->where('min_price','>=',$min_price)
            ->where('max_price','<=',$max_price)
            ->having('inventories_count', '>', 2)
            ->offset($start)->take(10)->get();
        }
        else
        {
            $listings = Product::whereIn('id',$ids)->with(['inventories.image:path,imageable_id','inventories.attributeValues' => function ($q) use ($attr) {
                $q->select('id', 'attribute_values.attribute_id', 'value', 'color')->whereIn('id',$attr);}])->withCount(['inventories' => function($query){
                $query->available();
            }])
            ->where('min_price','>=',$min_price)
            ->where('max_price','<=',$max_price)
        ...
        //return $listings;
        $data =ProductResource::collection($listings);
        
        return $this->processResponse('data',$data,'success','Filter data show successfully!!');
    } 
    else
         return $this->processResponse(null,null,'error','Enter correct login details'); 
    }
    ...
    {    
      ...
      { 
        ...
        $p = array("order_id"=>$order->id,"created_at"=>($order->created_at)->toDateTimeString(),"name"=>'Pending');
        $arr = array();
        array_push($arr,$p);
      ...
      {  
        ...
        switch($request->type)
        {
            ...
            case 'new_arrivals':
                ...
                $product_resource =ProductResource::collection($products);
                return $this->processResponse('data',$arr,'success','Products show successfully!!');
            break;
        }
    ...
    public function bank_details(Request $request)
    {
        ...
        if($cust_id)
        {
            $bank_details = Bank::where('customer_id',$cust_id->user_id)->first();
            if($bank_details != null)
            {
                $bank = Bank::find($bank_details->id);
    ...
    public function bank_details_show(Request $request)
    {
        $cust_id= DB::table('connection_request')->select('user_id')->where('connection_id','=', $request->connection_id)->first();
        if($cust_id)
        {
        ...
        return $this->processResponse('data',$bank_details,'success','Products show successfully!!');
    } 
    else
         return $this->processResponse(null,null,'error','Enter correct login details');
    }
