...
class CartController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        if($cust_id)
      {   
        $cart = Cart::select('id')->where('customer_id',$cust_id->user_id)->first();
        if($cart == null)
        {
         return response()->json(['status'=>'failed','message' => trans('Cart item not found')], 200);
        }
        $cart_id = $cart->id;
        ...
        $final_cart = new CartResource($cart);
        $p = array();
        array_push($p,$final_cart);
        return response()->json(['data'=>$p,'status'=>'success','message' => trans('Show Cart Successfully')], 200);
    ...
    public function addToCart(Request $request, $slug)
    {
        ...
        if($cust_id)
      {
        ...
        $customer_id = $cust_id->user_id ? $cust_id->user_id  : Null;
        if($customer_id){
        ...
        if($request->quantity > $item->stock_quantity) {
            return response()->json(['message' => trans('api.item_max_stock')], 200);
        }
        ...
        if($old_cart){
            ...
            if($item_in_cart) {
                return response()->json(['message' => trans('Item already in cart'),'status' => 'failed'], 200); // Item alrealy in cart
            }
        }
        ...
        return response()->json(['message' => trans('Item added into cart'),'status' => 'success'], 200);
       // return $this->processResponse('Category_sub_group',$category_sub_groups,'success','Category Sub Group Show Successfully!!');
    } 
    else
         return $this->processResponse(null,null,'error','Enter correct login details');
    ...
    public function update(ApiUpdateCartRequest $request, Cart $cart)
    {
        ...
        if($cust_id)
      {
        ...
        if($request->item && $request->quantity){
            ...
            // Check the available stock limit
            if($request->quantity > $item->stock_quantity) {
                return response()->json(['status'=>'failed','message' => trans('api.item_max_stock')], 200);
            }
    ...
    public function update_margin(Request $request, Cart $cart)
    {          
        $cust_id= DB::table('connection_request')->select('user_id')->where('connection_id','=', $request->connection_id)->where('auth_code','=',$request->auth_code)->first();
        if($cust_id)
      {  
    ...
    public function delete(Request $request, Cart $cart)
    {
      ...
      {  
       ...
       $result = \DB::table('cart_items')->whereIn('inventory_id', $items)->delete();
        ...
        return response()->json([
            'message' => trans('Cart deleted successfully'),
           // 'cart' => new CartResource($cart),
           'status'=> 'success',
        ], 200);
    } 
