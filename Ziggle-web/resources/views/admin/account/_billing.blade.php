		  		...
		  		<div class="panel-body">
					<fieldset>
						<legend>{{ trans('app.billing_info') }}</legend>
			  			@if($profile->shop->hasPaymentMethod())
			  				<h4>{{ trans('app.saved_card') }}: </h4>
			  				<span>{{$profile->shop->card_holder_name}}</span>
			  				<span class="pull-right">
				  				<i class="fa fa-cc-{{ strtolower($profile->shop->card_brand) }}"></i> ************{{$profile->shop->card_last_four}}
			  				</span>
			  				<br/><br/>
			  				<h4>{{ trans('app.update_card_info') }}: </h4>
			  			@endif
				        ...
				        {!! Form::model($profile, ['method' => 'PUT', 'route' => ['admin.account.card.update'], 'id' => 'stripe-form', 'data-toggle' => 'validator']) !!}
							...
							<div class="pull-right">
					            {!! Form::submit(trans('app.update'), ['id' => 'card-button', 'class' => 'btn btn-lg btn-new', 'data-secret' => $intent->client_secret]) !!}
							</div>
						{!! Form::close() !!}
					</fieldset>
				</div>
