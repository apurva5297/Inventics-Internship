<style type="text/css">
  ...
  .shopping-cart-item-table tbody {
          ...
          {!! Form::model($cart, ['method' => 'PUT', 'route' => ['cart.update', $cart->id], 'id' => 'formId'.$cart->id]) !!}
            ...
            <div class="col-md-9 nopadding">
                ...
                <table class="table shopping-cart-item-table" id="table{{$cart->id}}">
                    ...
                    <tbody>
                      <tr>
                          <th style="width:50px">{{ trans('theme.image') }}</th>
                          <th style="min-width:150px">{{ trans('theme.description') }}</th>
                          <th style="min-width:100px">{{ trans('theme.price') }}</th>
                          <th style="min-width:100px">{{ trans('theme.quantity') }}</th>
                          <th style="min-width:100px">{{ trans('theme.total') }}</th>
                      ...
                      @foreach($cart->inventories as $item)
                        ...
                        <tr class="cart-item-tr">
                          <td>
                            ...
                            <img src="{{ get_storage_file_url(optional($item->image)->path, 'mini') }}" alt="{{ $item->slug }}" title="{{ $item->slug }}" style="max-width: 30px" />
                          </td>
                          <td>
            ...
            <div class="col-md-3 space20">
              <div class="space30 hidden-sm hidden-md hidden-lg"></div>
                ...
                <button class="btn btn-primary btn-sm flat pull-right" id="checkout-btn{{$cart->id}}" type="submit"><i class="fa fa-shopping-cart"></i> {{ trans('Checkout') }}</button>
            </div> <!-- /.col-md-3 -->
          {!! Form::close() !!}
        </div> <!-- /.row -->
      @endforeach

      <a class="btn btn-black flat" href="@if(Session::get('shop')){{ url('/shop',Session::get('shop')->slug) }}@elseif(isset($shop)){{url('shop',$shop->slug)}}@else{{url('/')}}@endif">{{ trans('theme.button.continue_shopping') }}</a>
    @else
      <div class="clearfix space50"></div>
      <p class="lead text-center space50">
        @lang('theme.empty_cart')
        <a href="@if(Session::get('shop')){{ url('/shop',Session::get('shop')->slug) }}@elseif(isset($shop)){{url('shop',$shop->slug)}}@else{{url('/')}}@endif" class="btn btn-primary btn-sm flat">@lang('theme.button.shop_now')</a>
      </p>
    @endif
  </div> <!-- /.container -->
</section>