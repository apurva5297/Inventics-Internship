...
class ListHelper
{
    ...
    /**
     * Get list of all available Master category
     *
     * @return array
     */
    public static function masterCategory()
    {
        return \DB::table('master_categories')->where('deleted_at', Null)->orderBy('name', 'asc')->get();
    }
    ...
    public static function catWithSubGrpListArray()
    {
        ...
        if($shop_id)
        {
            $shop_category = ShopCategory::where('shop_id',$shop_id)->first();
            foreach(json_decode($shop_category->category_sub_group_id) as $cat_sub_grp_id)
            {
                $shop_sub_cat_id[] = $cat_sub_grp_id;
            }
            ...
            foreach ($categoryGroups as $categoryGroup){
                foreach ($categoryGroup->subGroups as $categorySubGroup){
                    ...
                    if(count($list))
                    {
                        if(count($shop_sub_cat_id) > 0 && in_array($categorySubGroup->id, $shop_sub_cat_id))
    ...
    {
        return Inventory::select('id','slug','title','condition','sale_price','offer_price','offer_start','offer_end','stuff_pick','set_size','set_desc','stock_quantity')
    ...
    {
        return Inventory::select('id','slug','title','condition','sale_price','offer_price','offer_start','offer_end','set_size','set_desc','stock_quantity')
        ->with(['feedbacks:rating,feedbackable_id,feedbackable_type', 'image:path,imageable_id,imageable_type'])
    ...
    public static function inventory_bin_stock($inventory_id=0)    
    {
        return \DB::table('inventory_bin_storage')->where('shop_id', Auth::user()->merchantId())
        ->where('inventory_id', $inventory_id)->where('active', 1)->sum('qty');
    }

    public static function inventory_stock()    
    {
        return \DB::table('inventories')->where('shop_id', Auth::user()->merchantId())
        ->count();
    }

    public static function getPricelist_name($id)
    {
        if ($id==0) {
            return 'Sale Price';
        }elseif($id==00){
            ...
            $result=\DB::table('pricelist')->where('id', $id)->first();
            return $result->name;
        }
    }
