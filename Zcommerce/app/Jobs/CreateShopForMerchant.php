...
class CreateShopForMerchant
{
    ...
    public function handle()
    {
        $shop = Shop::create([
            ...
            'current_billing_plan' => isset($this->request['plan']) ? $this->request['plan'] : 'free',
            'trial_ends_at' => (bool) config('system_settings.trial_days') ? now()->addDays(config('system_settings.trial_days')) : date('Y-m-d H:i:s', strtotime('+1 years')),
            'active' => isset($this->request['active']) ? $this->request['active'] : 1,
            'pan' => isset($this->request['pan']) ? $this->request['pan'] : null,
            'gstin' => isset($this->request['gstin']) ? $this->request['gstin'] : Null,
        ]);
