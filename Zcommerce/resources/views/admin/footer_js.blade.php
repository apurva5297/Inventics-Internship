<script type="text/javascript">
	...
	**************************************/
	...
	//post data
	$(document).on('click', '#mappnow', function () {
            if ($('.prolist:checked').length > 0) {
            	var list = [];
	            $.each($(".prolist:checked"), function(){            
	                list.push($(this).attr('id'));
	            });
	            /*var category=$('.select2-selection__choice').attr('title');
	            alert("My favourite sports are: " + JSON.stringify(list));
	            alert(category);*/
               	$.post("http://206.189.141.143/api/mapping", {
					products:JSON.stringify(list),
					category:$('.select2-selection__choice').attr('title'),
                }, function (data) {
                    alert(data);
                    location.reload();
                });
	...
	function initDatatables()
	{
	    ...
	    $('#all-mapping-table').DataTable({
		  	"aaSorting": [],
		    "iDisplayLength": {{ getPaginationValue() }},
	        "processing": true,
	        "serverSide": true,
	        "ajax": "{{ route('admin.catpro.mapping.getMore') }}",
			"columns": [
	            { 'data': 'name', 'name': 'name' },
	            { 'data': 'gtin', 'name': 'gtin' },
	            { 'data': 'model_number', 'name': 'model_number' },
		...
		$('#all-customer-table').DataTable({
	  	...
	  	$(".table-bin").DataTable({
		    ...
		    "bprocessing": true,
	        "serverSide": true,
	        "ajax": "{{ route('admin.stock.warehouse.consolidate_data') }}",
		    "oLanguage": {
		        "sInfo": "_START_ to _END_ of _TOTAL_ entries",
		        "sLengthMenu": "Show _MENU_",
		        "sSearch": "",
	...
	**************************************/
	...
	function addMoreBinRow(e)
	{
		var bin= $('#get-more-bin').html();
		$('#add-more-bin').append('<div class="row">'+bin+'</div>');
		//console.log(bin);
	}

	//bin inventory Seach
	$('#binsearchInventory').on('keyup', function(e){
	  	var showResult = $("#productFounds");
	  	var q = $(this).val();
	  	showResult.html('');
	  	if(q.length < 3){
	...
	function addToBin(e)
	{	
		var showResult = $("#inventoryFounds");
		showResult.html('');
		var id = $(e).data('id');
		$('#binsearchInventory').val('');
		$('#inventoryFounds').html('');
		showResult.html('<span class="lead indent50">{{trans('responses.searching')}}</span>');
		$.ajax({
			data: "id="+id,
	      	url : "{{ route('search.add-to-bin') }}",
	      	success: function(results){
	      		$('#productFounds').html('');
			  	showResult.html(results);
	      	}
...
<script type="text/javascript">
	$( document ).ready(function() {
    //         $('#product_consolidate_grid_all').DataTable({
    //             "bProcessing": true,
    //             "serverSide": true,
    //             "scrollY": 400,
    //             "scrollX": true,
    //             "bStateSave": true,
    //             "ajax":{
    //                 url :"{{ url('/admin/catalog/product_consolidate_data') }}", // json datasource
    //                 type: "post", 
    //                 data: {
    //                 		"_token": "{{ csrf_token() }}"
    //                 },
    ...
    //                       $("#consolidate_processing").css("display","none");
    //                       alert(data.responseText);
    //                 },
    //             },
    //             "columns": [
	   //              	{ 'data': '0', 'name': 'products.name', 'orderable': true, 'searchable': true },
			 //            { 'data': '1', 'name': 'shops.name', 'orderable': true, 'searchable': true },
			 //            { 'data': '2', 'name': 'products.model_number', 'orderable': true, 'searchable': true },
			 //            { 'data': '3', 'name': 'categories.name', 'orderable': true, 'searchable': false },
			 //            { 'data': '4', 'name': 'inventories_count', 'orderable': true, 'searchable': true },
			 //            { 'data': '5', 'name': 'products.admin_approval', 'orderable': true, 'searchable': true },
			 //            { 'data': '6', 'name': 'option', 'orderable': false, 'searchable': false, 'exportable': false}
		  //           ],
...
var last_part = parts[parts.length-1];
    	$('#bin_ajax_list').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('/admin/stock/index-bin/bin_consolidate_data') }}",
                     "dataType": "json",
                     "type": "POST",
                     "data":{ _token: "{{csrf_token()}}" , "warehouse_zone_group_id": last_part}
                   },
            "columns": [
                { "data": "name" },
                { "data": "code" },
        ...
        $('#product_ajax_list').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('/admin/catalog/product_consolidate_data') }}",
                     "dataType": "json",
                     "type": "POST",
        ...
        $('#active_inventory').DataTable({
            ...
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('admin/stock/ajax_inventory/active') }}",
                     "dataType": "json",
                     "type": "POST",
                     "data":{ _token: "{{csrf_token()}}"}
                   },
        ...
        var pricelist_csv_type= $('#pricelist_csv_id').val();
        $('#pricelist_inventory').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('admin/stock/pricelist_inventory') }}"+'/'+pricelist_csv_type,
                     "dataType": "json",
                     "type": "POST",
                     "data":{ _token: "{{csrf_token()}}"}
                   },
        ...
        console.log(pricelist_csv_type);
        $('#marketplace_inventory').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('admin/stock/pricelist_inventory') }}"+'/'+pricelist_csv_type,
         ...
         $('#marketplace_listing_id').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
                     "url": "{{ url('admin/stock/pricelist_inventory') }}"+'/'+pricelist_csv_type,
            ...
            "columns": [
                ...
                { "data": "title" },
                { "data": "condition" },
                { "data": "flipkart_serial_number" },
                { "data": "listing_id" },
                { "data": "your_selling_price" },
            ],
            dom: 'Bfrtip',
			    buttons: [
			        	'pageLength', 'copy', 'csv', 'excel', 'pdf', 'print'
	...
	function changeReportType(e)
	{
		...
		$.ajax({
			data: "id="+id,
	      	url : "{{ url('admin/stock/marketplace-report-type') }}",
	      	success: function(results){
	      		$('.report-type-section').show();
			  	$('#table_field').html(results);
	      	}
        });
	}
</script>